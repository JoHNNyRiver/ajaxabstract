...
const Ajax = (function () {
   ...
   */
  const helperHttp = (method, uri, fn, data = null) => {
    ...
    xhr.onerror = () => {
      ...
      fn(null, {message: `Erro ao estabelecer conexão com ${uri}`, status: xhr.status})
    }
      ...
      if (xhr.status === 200) {
        ...
        switch (mimiType) {
          case 'application/json':
            ...
            break
          case 'text/html':
            const parserDom = new window.DOMParser()
            parser = parserDom.parseFromString(xhr.responseText, 'text/html')
            break
          case 'application/xml':
            const parserXML = new window.DOMParser()
        ...
        fn({data: xhr.responseText, parsed: parser, status: xhr.status, statusText: xhr.statusText, time: scope.progress, mimitype: mimiType}, null)
      } else {
        fn(null, {message: xhr.responseText, status: xhr.status, time: scope.progress})
      }
    }
